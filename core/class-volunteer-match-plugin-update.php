...
if ( ! class_exists( 'Tribe_Events_Enforcer_Plugin_Update' ) ) {
	/**
	 * TribeEventsEnforcer Plugin Upgrader
	 */
	class Tribe_Events_Enforcer_Plugin_Update {
		 ...
		 */
		protected $transient_name = 'tribe_events_enforcer_update_plugins';
		 ...
		 */
		protected $repo = 'https://api.github.com/repos/Danny-Guzman/events-calendar-enforcer';
		...
		public function __construct( $plugin_slug ) {
			...
			add_filter( 'pre_set_site_transient_update_plugins', array( $this, 'tribe_events_enforcer_check_plugin_update' ) );

			// Define the alternative response for information checking.
			add_filter( 'site_transient_update_plugins', array( $this, 'tribe_events_enforcer_add_plugins_to_update_notification' ) );

			add_filter( 'plugins_api', array( $this, 'tribe_events_enforcer_update_plugins_changelog' ), 20, 3 );
			...
			add_filter( 'upgrader_source_selection', array( $this, 'tribe_events_enforcer_upgrader_source_selection' ), 10, 4 );
		 ...
		 * @return array
		 */
		public function tribe_events_enforcer_check_plugin_update( $update_transient ) {
		...
		/**
		 * Adds the EventsCalendarEnforce Plugin Update Notification to List of Available Updates.
		 *
		 * @param  array $update_transient Transient containing plugin updates.
		 *
		 * @return array
		 */
		public function tribe_events_enforcer_add_plugins_to_update_notification( $update_transient ) {
			$tribe_events_enforcer_update_plugins = get_site_transient( $this->transient_name );
			if ( ! is_object( $tribe_events_enforcer_update_plugins ) || ! isset( $tribe_events_enforcer_update_plugins->response ) ) {
				return $update_transient;
			}
			...
			$update_transient->response = array_merge(
				...
				$tribe_events_enforcer_update_plugins->response
			);
		 ...
		 */
		public function tribe_events_enforcer_update_plugins_changelog( $result, $action, $args ) {
			if ( isset( $args->slug ) && $args->slug === $this->slug ) {
				$tribe_events_enforcer_update_plugins = get_site_transient( $this->transient_name );
				if ( isset( $tribe_events_enforcer_update_plugins->response ) && isset( $tribe_events_enforcer_update_plugins->response[ $this->plugin_file ] ) ) {
					$tmp = $this->plugin_details();

					$tmp['version']      = $tribe_events_enforcer_update_plugins->response[ $this->plugin_file ]->new_version;
					$tmp['last_updated'] = $tribe_events_enforcer_update_plugins->response[ $this->plugin_file ]->published_date;

					$tmp['sections']['Changelog'] = $this->tribe_events_enforcer_get_plugin_changelog( $tmp['version'] );
		 ...
		 * @return string
		 */
		public function tribe_events_enforcer_get_plugin_changelog( $ver = 'master' ) {
		 ...
		 * @return string
		 */
		public function tribe_events_enforcer_upgrader_source_selection( $src, $rm_src, $upgr, $options ) {
		...
		public function plugin_details() {
			$view_details = array(
				...
				'author'   => 'Danny Guzman',
				'name'     => 'Force WP Template for The Events Calendar Plugin',
				'sections' => array(
...
new Tribe_Events_Enforcer_Plugin_Update( plugin_basename( plugin_dir_path( __DIR__ ) ) );
